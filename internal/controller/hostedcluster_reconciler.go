...
func updateCIProvisionedStatusFromHostedCluster(
	...
	// - Degraded = True OR ClusterVersionFailing = True
	// - Progressing = False
	if (degraded.Status == metav1.ConditionTrue || isConditionTrue(clusterVersionFailing)) &&
		progressing.Status == metav1.ConditionFalse {
		conditions.SetStatusCondition(&ci.Status.Conditions,
			v1alpha1.ClusterProvisioned,
			v1alpha1.Failed,
...
//   - Failed: Degraded or ClusterVersionFailing
...
func updateCIDeploymentConditionsFromHostedCluster(
	...
	// Calculate Failed: Degraded=True OR ClusterVersionFailing=True
	// If either condition exists and is True, installation has failed
	failed := isConditionTrue(degraded) || isConditionTrue(clusterVersionFailing)
