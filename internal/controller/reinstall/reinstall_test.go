...
var _ = Describe("finalizeReinstallRequest", func() {
	...
	It("should set the ReinstallHistory", func() {
		...
		lastAppliedBytes, _ := json.Marshal(lastApplied)
		metav1.SetMetaDataAnnotation(&clusterInstance.ObjectMeta, corev1.LastAppliedConfigAnnotation, string(lastAppliedBytes))
	...
	It("should return an error if the ClusterInstance.Spec diff cannot be computed", func() {
		metav1.SetMetaDataAnnotation(&clusterInstance.ObjectMeta, corev1.LastAppliedConfigAnnotation, "error")
		Expect(c.Update(ctx, clusterInstance)).To(Succeed())
