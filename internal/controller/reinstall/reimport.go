...
const (
	...
	// AnnotationImmediateImport is the annotation used to trigger immediate import of a managed cluster
	AnnotationImmediateImport = "open-cluster-management/immediateimport"
...
func (r *ReinstallHandler) EnsureClusterIsReimported(
	...
	// Calculate time since provisioning completed
	timeSinceProvisioning := time.Since(provisionedCondition.LastTransitionTime.Time)
	gracePeriodRemaining := PostProvisioningGracePeriod - timeSinceProvisioning
	...
	if inGracePeriod {
		log.Info("Cluster unhealthy but still in grace period, waiting for auto-recovery",
			zap.Duration("gracePeriodRemaining", remaining),
	...
	if isImportInProgress(managedCluster) {
		log.Info("Import is already in progress, waiting for completion",
			zap.String("ImportReason", healthStatus.importReason))
		reimportNeeded = true
		result = reconcile.Result{Requeue: true, RequeueAfter: 30 * time.Second}
		clusterReimportedCondition = reinstallClusterReimportedConditionStatus(
			metav1.ConditionFalse, v1alpha1.InProgress,
	...
	// Check if reimport already triggered
	if isReimportAlreadyTriggered(managedCluster) {
		annotationValue := managedCluster.Annotations[AnnotationImmediateImport]
		log.Info("Reimport already triggered, waiting for completion",
			zap.String("AnnotationValue", annotationValue))
		reimportNeeded = true
		result = reconcile.Result{Requeue: true, RequeueAfter: 30 * time.Second}
		clusterReimportedCondition = reinstallClusterReimportedConditionStatus(
			metav1.ConditionFalse, v1alpha1.InProgress,
			"Waiting for cluster reimport to complete")
		return
	}
	...
	// Reimport initiated, wait for completion
	log.Info("Reimport initiated successfully")
	reimportNeeded = true
	result = reconcile.Result{Requeue: true, RequeueAfter: 30 * time.Second}
	clusterReimportedCondition = reinstallClusterReimportedConditionStatus(
		metav1.ConditionFalse, v1alpha1.InProgress,
		"Cluster reimport initiated, waiting for completion")
...
func (r *ReinstallHandler) performReimport(
	...
	// Also create auto-import-secret and annotation for completeness
	// This ensures OCM import controller has the resources it expects
	log.Info("Creating auto-import-secret", zap.String("cluster", clusterName))
	if err := r.createAutoImportSecret(ctx, clusterName); err != nil {
		return fmt.Errorf("failed to create auto-import-secret: %w", err)
	}
	log.Info("Auto-import secret ready", zap.String("cluster", clusterName))
	log.Info("Adding immediate import annotation", zap.String("cluster", clusterName))
	patch := client.MergeFrom(managedCluster.DeepCopy())
	metav1.SetMetaDataAnnotation(&managedCluster.ObjectMeta,
		AnnotationImmediateImport, "")
	if err := r.Client.Patch(ctx, managedCluster, patch); err != nil {
		return fmt.Errorf("failed to add immediate import annotation: %w", err)
	}
	log.Info("Immediate import annotation added", zap.String("cluster", clusterName))

	return nil
}

// getSpokeClient creates a Kubernetes client for the spoke cluster using the admin kubeconfig.
// This is a convenience function that uses the DefaultSpokeClientFactory.
//
//nolint:unparam
func getSpokeClient(ctx context.Context, c client.Client, clusterName string) (client.Client, error) {
	// Get the admin kubeconfig secret
	kubeconfigData, err := extractKubeconfigFromSecret(ctx, c, clusterName)
	if err != nil {
		return nil, err
	}

	// Use the default factory to create the client
	factory := &DefaultSpokeClientFactory{}
	return factory.CreateClient(ctx, kubeconfigData)
}
...
// createAutoImportSecret creates an auto-import-secret by extracting
// kubeconfig from the admin-kubeconfig secret in the managed cluster's namespace.
func (r *ReinstallHandler) createAutoImportSecret(
	ctx context.Context,
	clusterName string,
) error {

	// Check if auto-import-secret already exists (idempotency)
	...
	if !apierrors.IsNotFound(err) {
		return fmt.Errorf("failed to check for existing auto-import-secret: %w", err)
	}
	...
	if err := r.Client.Create(ctx, autoImportSecret); err != nil {
...
// isReimportAlreadyTriggered checks if reimport was already triggered by checking the annotation status.
func isReimportAlreadyTriggered(mc *clusterv1.ManagedCluster) bool {
	if annotationValue, exists := mc.Annotations[AnnotationImmediateImport]; exists {
		// Empty or "Completed" means we should trigger new import
		return annotationValue != "" && annotationValue != "Completed"
	}
	return false
}
