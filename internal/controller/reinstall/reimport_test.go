...
func (b *testManagedClusterBuilder) WithImportAnnotation(value string) *testManagedClusterBuilder {
	b.mc.Annotations[AnnotationImmediateImport] = value
	return b
}
...
`)
	...
	Context("when reimport already triggered via annotation", func() {
		BeforeEach(func() {
			provisionedTime := time.Now().Add(-15 * time.Minute)
			clusterInstance = newTestClusterInstance("test-cluster", "test-ns").
				WithReinstallSpec("gen-1", v1alpha1.PreservationModeNone).
			...
			managedCluster = newTestManagedCluster("test-cluster").
				WithUnhealthyAvailableCondition().
				WithImportAnnotation("InProgress").
				Build()
		...
		It("should wait for reimport completion with reimportNeeded=true", func() {
			result, reimportNeeded, err := handler.EnsureClusterIsReimported(ctx, testLogger, clusterInstance)
			...
			Expect(condition.Status).To(Equal(metav1.ConditionFalse))
			Expect(condition.Reason).To(Equal(string(v1alpha1.InProgress)))
			Expect(condition.Message).To(ContainSubstring("Waiting for cluster reimport to complete"))
		})
	})
...
var _ = Describe("createAutoImportSecret", func() {
	var (
		ctx        context.Context
		testLogger *zap.Logger
	)
	...
	It("should return nil when auto-import-secret already exists (idempotent)", func() {
		...
		err := handler.createAutoImportSecret(ctx, "test-cluster")

		Expect(err).NotTo(HaveOccurred())

		secret := &corev1.Secret{}
		err = c.Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "auto-import-secret"}, secret)
		Expect(err).NotTo(HaveOccurred())
		Expect(string(secret.Data["kubeconfig"])).To(Equal("existing-kubeconfig"))
	})

	It("should return error when admin kubeconfig secret does not exist", func() {
		c := fakeclient.NewClientBuilder().
			WithScheme(scheme.Scheme).
			Build()
		handler := &ReinstallHandler{
	...
	It("should return validation error when admin kubeconfig secret has invalid type", func() {
	...
	It("should create auto-import-secret with kubeconfig data from valid admin secret", func() {
		...
		Expect(string(secret.Data["kubeconfig"])).To(Equal("valid-kubeconfig-data"))
	})

	It("should return error when admin kubeconfig has no valid kubeconfig key", func() {
		adminSecret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
				Namespace: "test-cluster",
	...
	It("should return error when checking for existing secret fails unexpectedly", func() {
		mockCtrl := gomock.NewController(GinkgoT())
		defer mockCtrl.Finish()
		mockClient := mocks.NewGeneratedMockClient(mockCtrl)

		mockClient.EXPECT().
			Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "auto-import-secret"}, gomock.Any()).
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("failed to check for existing auto-import-secret"))
	})

	It("should return nil when Create returns AlreadyExists (race condition)", func() {
		mockCtrl := gomock.NewController(GinkgoT())
		defer mockCtrl.Finish()
		mockClient := mocks.NewGeneratedMockClient(mockCtrl)
		...
		mockClient.EXPECT().
			Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "auto-import-secret"}, gomock.Any()).
			Return(apierrors.NewNotFound(schema.GroupResource{Resource: "secrets"}, "auto-import-secret"))
		mockClient.EXPECT().
			Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "test-cluster-admin-kubeconfig"}, gomock.Any()).
			DoAndReturn(func(_ context.Context, _ client.ObjectKey, obj client.Object, _ ...client.GetOption) error {
				secret := obj.(*corev1.Secret)
				secret.Name = "test-cluster-admin-kubeconfig"
				secret.Namespace = "test-cluster"
		...
		mockClient.EXPECT().
			...
			Return(apierrors.NewAlreadyExists(schema.GroupResource{Resource: "secrets"}, "auto-import-secret"))
	...
	It("should return error when Create fails with non-AlreadyExists error", func() {
		mockCtrl := gomock.NewController(GinkgoT())
		defer mockCtrl.Finish()
		mockClient := mocks.NewGeneratedMockClient(mockCtrl)
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("failed to create auto-import-secret"))
	})
})

var _ = Describe("getSpokeClient", func() {
	var (
		ctx context.Context
		c   client.Client
	)
	...
	It("should return error when admin kubeconfig secret does not exist", func() {
		...
		spokeClient, err := getSpokeClient(ctx, c, "test-cluster")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("failed to get admin kubeconfig secret"))
		Expect(spokeClient).To(BeNil())
	})

	It("should return error when secret has invalid type", func() {
		secret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
				Namespace: "test-cluster",
			},
			Type: corev1.SecretTypeTLS, // Wrong type
			Data: map[string][]byte{
				"kubeconfig": []byte("some-data"),
			},
		}
	...
	It("should return error when secret has wrong namespace", func() {
		secret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
	...
	It("should return error when kubeconfig data is missing", func() {
		secret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
				Namespace: "test-cluster",
	...
	It("should return error when kubeconfig is not valid YAML", func() {
		secret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
				Namespace: "test-cluster",
			...
			Type: corev1.SecretTypeOpaque,
			Data: map[string][]byte{
				"kubeconfig": []byte("not-valid-yaml: {{{"),
			},
		}
		...
		spokeClient, err := getSpokeClient(ctx, c, "test-cluster")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("failed to create REST config from kubeconfig"))
		Expect(spokeClient).To(BeNil())
	})

	It("should return error when kubeconfig is valid YAML but invalid kubeconfig format", func() {
		invalidKubeconfig := []byte(`
apiVersion: v1
kind: ConfigMap
metadata:
...
`)
	...
	It("should return error when kubeconfig has invalid server URL", func() {
		invalidServerKubeconfig := []byte(`
apiVersion: v1
kind: Config
clusters:
- cluster:
...
`)
		...
		secret := &corev1.Secret{
			...
			Type: corev1.SecretTypeOpaque,
			Data: map[string][]byte{
				"kubeconfig": invalidServerKubeconfig,
			},
		}
		...
		spokeClient, err := getSpokeClient(ctx, c, "test-cluster")
		Expect(err).To(HaveOccurred())
		// Will fail either at REST config creation or client creation
		Expect(spokeClient).To(BeNil())
	})

	It("should return error when kubeconfig is empty", func() {
		secret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-cluster-admin-kubeconfig",
				Namespace: "test-cluster",
	...
	It("should handle context cancellation during client creation", func() {
		// Create a kubeconfig that would require network access
		validFormatKubeconfig := []byte(`
apiVersion: v1
kind: Config
clusters:
- cluster:
    server: https://unreachable-cluster.example.com:6443
    insecure-skip-tls-verify: true
  name: test-cluster
contexts:
- context:
...
`)
		...
		secret := &corev1.Secret{
			...
			Type: corev1.SecretTypeOpaque,
			Data: map[string][]byte{
				"kubeconfig": validFormatKubeconfig,
			},
		}
		...
		// This will fail at validateSpokeConnectivity since the spoke cluster doesn't exist
		spokeClient, err := getSpokeClient(ctx, c, "test-cluster")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("spoke cluster connectivity validation failed"))
		Expect(spokeClient).To(BeNil())
...
var _ = Describe("performReimport", func() {
	...
	Context("when spoke client is successfully created", func() {
		...
		It("should complete full reimport flow successfully", func() {
			...
			err = c.Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "auto-import-secret"}, autoImportSecret)
			Expect(err).NotTo(HaveOccurred())
			Expect(autoImportSecret.Data["kubeconfig"]).To(Equal([]byte("valid-kubeconfig-data")))

			updatedMC := &clusterv1.ManagedCluster{}
			err = c.Get(ctx, client.ObjectKey{Name: "test-cluster"}, updatedMC)
			Expect(err).NotTo(HaveOccurred())
			Expect(updatedMC.Annotations).To(HaveKey(AnnotationImmediateImport))
		})

		It("should fail when auto-import-secret creation fails", func() {
			crdsManifestWork := createTestManifestWork("test-cluster", "test-cluster-klusterlet-crds",
				[]map[string]interface{}{createTestNamespaceManifest("open-cluster-management-agent")})
			klusterletManifestWork := createTestManifestWork("test-cluster", "test-cluster-klusterlet",
				[]map[string]interface{}{createTestConfigMapManifest("klusterlet-config", "open-cluster-management-agent")})
			...
			mockHubClient := mocks.NewGeneratedMockClient(mockCtrl2)

			mockHubClient.EXPECT().
				Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "test-cluster-admin-kubeconfig"}, gomock.Any()).
				DoAndReturn(func(_ context.Context, _ client.ObjectKey, obj client.Object, _ ...client.GetOption) error {
					secret := obj.(*corev1.Secret)
					secret.Name = "test-cluster-admin-kubeconfig"
					secret.Namespace = "test-cluster"
					secret.Type = corev1.SecretTypeOpaque
					secret.Data = map[string][]byte{"kubeconfig": []byte("valid-kubeconfig-data")}
					return nil
				})

			mockHubClient.EXPECT().
				Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "test-cluster-klusterlet-crds"}, gomock.Any()).
				DoAndReturn(func(_ context.Context, _ client.ObjectKey, obj client.Object, _ ...client.GetOption) error {
					mw := obj.(*unstructured.Unstructured)
					mw.SetUnstructuredContent(crdsManifestWork.UnstructuredContent())
					return nil
				})

			mockHubClient.EXPECT().
				Get(ctx, client.ObjectKey{Namespace: "test-cluster", Name: "test-cluster-klusterlet"}, gomock.Any()).
				DoAndReturn(func(_ context.Context, _ client.ObjectKey, obj client.Object, _ ...client.GetOption) error {
					mw := obj.(*unstructured.Unstructured)
					mw.SetUnstructuredContent(klusterletManifestWork.UnstructuredContent())
					return nil
				})
			...
			// Expect manifests to be applied
			mockSpokeClient.EXPECT().
				Patch(ctx, gomock.Any(), client.Apply, gomock.Any(), gomock.Any()).
				Return(nil).Times(2)
...
var _ = Describe("isReimportAlreadyTriggered", func() {
	DescribeTable("should evaluate annotation correctly",
		func(annotationValue string, hasAnnotation bool, expected bool) {
			annotations := map[string]string{}
			if hasAnnotation {
				annotations[AnnotationImmediateImport] = annotationValue
			}
			mc := &clusterv1.ManagedCluster{
				ObjectMeta: metav1.ObjectMeta{Annotations: annotations},
			}
			Expect(isReimportAlreadyTriggered(mc)).To(Equal(expected))
		},
		Entry("InProgress value", "InProgress", true, true),
		Entry("empty value", "", true, false),
		Entry("Completed value", "Completed", true, false),
		Entry("no annotation", "", false, false),
	)
})
