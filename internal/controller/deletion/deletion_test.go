...
var _ = Describe("DeleteObject", func() {
	...
	BeforeEach(func() {
		...
		deletionCtx = NewDeletionContext(ctx, c, testLogger, nil)
	...
	It("should return true if the object does not exist", func() {
		...
		deleted, err := deletionCtx.DeleteObject(object, ownerRef)
	...
	It("should return an error if deletion times out", func() {
		...
		deletionCtx = NewDeletionContext(ctx, testClient, testLogger, nil)
	...
	It("should return false and wait if the object is marked for deletion but not yet deleted", func() {
		...
		deletionCtx.TimeoutDuration = nil // No timeout for this test
		deleted, err := deletionCtx.DeleteObject(object, ownerRef)
		Expect(err).ToNot(HaveOccurred())
		Expect(deleted).To(BeFalse())
	})
...
var _ = Describe("DeleteObjects", func() {
	...
	It("should return true if all objects are successfully deleted", func() {
		...
		deleted, err := deletionCtx.DeleteObjects(clusterInstance, []ci.RenderedObject{*renderedObject1, *renderedObject2}, nil)
		Expect(err).ToNot(HaveOccurred())
		Expect(deleted).To(BeFalse())
	...
	It("should exclude objects specified in the excludeObjects list", func() {
		...
		deleted, err := deletionCtx.DeleteObjects(clusterInstance, []ci.RenderedObject{*renderedObject1, *excludeRenderedObject},
			[]ci.RenderedObject{*excludeRenderedObject})
	...
	It("should return false if any object deletion fails", func() {
		...
		deleted, err := deletionCtx.DeleteObjects(clusterInstance, []ci.RenderedObject{*renderedObject}, nil)
		Expect(err).To(HaveOccurred())
...
var _ = Describe("DeleteRenderedObjects", func() {
	...
	It("should return true if all rendered objects are successfully deleted", func() {
		...
		deleted, err := deletionCtx.DeleteRenderedObjects(clusterInstance, nil)
		Expect(err).ToNot(HaveOccurred())
		Expect(deleted).To(BeFalse())
...
var _ = Describe("Utility Functions", func() {
	...
	Describe("buildDeletionObjects", func() {
		It("should correctly build deletion objects from ClusterInstance", func() {
			objects := deletionCtx.buildDeletionObjects(clusterInstance)
	...
	Describe("initiateObjectDeletion", func() {
		...
		It("should initiate deletion successfully", func() {
			...
			// Initiate Deletion
			Expect(deletionCtx.initiateObjectDeletion(obj)).To(Succeed())
		...
		It("should handle not found errors gracefully", func() {
			...
			err := deletionCtx.initiateObjectDeletion(obj)
			Expect(apierrors.IsNotFound(err)).To(BeFalse())
		})
	})

	Describe("ensureObjectExists", func() {

		It("should return false if the object does not exist", func() {
			obj := &unstructured.Unstructured{}
			obj.SetGroupVersionKind(gvk)
			obj.SetName("non-existent")
			obj.SetNamespace(Namespace)

			exists, err := deletionCtx.ensureObjectExists(obj)
			Expect(err).ToNot(HaveOccurred())
			Expect(exists).To(BeFalse())
		})

		It("should return true if the object exists", func() {
			obj := &unstructured.Unstructured{}
			obj.SetGroupVersionKind(gvk)
			obj.SetName(Name)
			obj.SetNamespace(Namespace)
