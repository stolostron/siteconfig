		...
		for _, object := range objectSyncWaveMap.GetSlice(syncWave) {
			...
			if err != nil {
				...
				if cierrors.IsDeletionTimeoutError(err) {
					manifestRef.UpdateStatus(v1alpha1.ManifestDeletionTimedOut, err.Error())
					return
				}
				manifestRef.UpdateStatus(v1alpha1.ManifestDeletionFailure, err.Error())
				continue
			}
			...
			if manifestRef.Status != v1alpha1.ManifestDeletionInProgress {
				...
				manifestRef.UpdateStatus(v1alpha1.ManifestDeletionInProgress, "")
			}
