...
func generateNodeMatchingKey(node NodeSpec) (string, error) {
	// Use hostname as primary key if available
	if node.HostName != "" {
		return "host:" + node.HostName, nil
	}
	// Use BMC address as secondary key
	if node.BmcAddress != "" {
		return "bmc:" + node.BmcAddress, nil
	}
	// Use boot MAC as tertiary key
	if node.BootMACAddress != "" {
		return "mac:" + node.BootMACAddress, nil
	}
	// Error if no unique identifiers are available
	return "", fmt.Errorf("node has no unique identifiers (hostname, BMC address, or boot MAC address)")
}

// areSameNodeSets checks if two node arrays are identical
func areSameNodeSets(oldNodes, newNodes []NodeSpec) bool {
	if len(oldNodes) != len(newNodes) {
		return false
	}

	// Quick check using JSON marshaling
	oldJSON, _ := json.Marshal(oldNodes)
	newJSON, _ := json.Marshal(newNodes)
...
func getNodeIdentifier(node NodeSpec) string {
	...
	if node.BmcAddress != "" {
		return node.BmcAddress
	}
	if node.BootMACAddress != "" {
		return node.BootMACAddress
	}
	return "unknown"
}
...
func getReinstallPermissibleFields() []string {
	var fields []string
	for field := range ReinstallPermissions {
...
func getReinstallNodePaths() []string {
	var paths []string
	for _, path := range ReinstallPermissions {
