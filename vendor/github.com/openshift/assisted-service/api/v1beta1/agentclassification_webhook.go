...
func (r *AgentClassification) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	newAgentClassification, ok := newObj.(*AgentClassification)
	if !ok {
		return nil, fmt.Errorf("new object is not an AgentClassification")
	}
	agentclassificationlog.Info("validate update", "name", newAgentClassification.Name)
	...
	if (oldAgentClassification.Spec.LabelKey != newAgentClassification.Spec.LabelKey) || (oldAgentClassification.Spec.LabelValue != newAgentClassification.Spec.LabelValue) {
