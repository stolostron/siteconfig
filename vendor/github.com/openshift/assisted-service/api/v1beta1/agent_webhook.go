...
func (r *Agent) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	newAgent, ok := newObj.(*Agent)
	if !ok {
		return nil, fmt.Errorf("new object is not an Agent")
	}
	agentlog.Info("validate update", "name", newAgent.Name)
	oldObject, ok := oldObj.(*Agent)
	if !ok {
		return nil, fmt.Errorf("old object is not an Agent")
	}
	if !areClusterRefsEqual(oldObject.Spec.ClusterDeploymentName, newAgent.Spec.ClusterDeploymentName) {
		...
		if funk.ContainsString(installingStatuses, newAgent.Status.DebugInfo.State) {
